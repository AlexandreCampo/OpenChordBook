...
class iRealRenderer {
	...
	parse(song) {
		var text = song.music;
		if (typeof text !== 'string' || text.length > 16384) throw new Error('Chart too large (16384 characters maximum).');
		var arr = [], headers = [], comments = [];
		var i;
		text = text.trimRight();
		while(text) {
			if (arr.length > 8192) throw new Error('Chart has too many tokens.');
			var found = false;
...
//		console.log(out);
	...
	parseChord(match, depth = 0) {
		if (depth > 4) throw new Error('Too many nested alternate chords.');
		var note = match[1] || " ";
		...
		if (alternate) {
			...
			if (!match)
				...
				alternate = this.parseChord(match, depth + 1);
		}
		// empty cell?
	...
	newToken(arr) {
		if (arr.length >= 2048) throw new Error('Chart has too many cells (2048 maximum).');
 ...
 * @type RegExp
 */
iRealRenderer.chordRegex = /^([ A-GW][b#]?)((?:sus|[\+\-\^\dhob#])*)(\*[^*]{1,512}\*)*(\/[A-G][#b]?)?(\(.*?\))?/;
