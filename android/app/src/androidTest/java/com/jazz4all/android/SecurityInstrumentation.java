...
public final class SecurityInstrumentation extends Instrumentation {
    private WebView web;
    private int checks;
    private static final String START = "https://appassets.androidplatform.net/assets/v13/web/index.html";
    ...
    @Override public void onStart() {
        ...
        try {
            ...
            await("typeof window.jazz4allNative?.fetchPlaylist === 'function' && !!window.jazz4allBack");
            check("\"object\"".equals(js("typeof window.jazz4allHost")), "Origin-bound transport available");
            check("false".equals(js("'getClass' in window.jazz4allNative")), "No Java reflection interface");
            js("window.nativeResult = ''; window.jazz4allNative.fetchPlaylist('https://127.0.0.1/secret').then(()=>window.nativeResult='BAD',e=>window.nativeResult=e.message); true");
            await("window.nativeResult.includes('Unknown catalog entry')");
            checks++;

            js("jazz4allHost.postMessage('{'); jazz4allHost.postMessage('x'.repeat(513)); try { jazz4allHost.postMessage(new ArrayBuffer(8)); } catch {} true");
            ...
            runOnMainSync(() -> {
                ...
                for (String url : new String[] { "https://appassets.androidplatform.net:444/assets/v13/web/index.html", "https://appassets.androidplatform.net/assets/v13/web/data/catalog.json", "data:text/html,hi", "javascript:alert(1)", "intent://scan/#Intent;scheme=zxing;end", "file:///etc/hosts", "content://contacts/people", "http://example.com/" }) {
                ...
                for (String url : new String[] { "https://example.com/", "https://appassets.androidplatform.net:444/assets/v13/web/index.html", "https://appassets.androidplatform.net/assets/v13/web/%2e%2e/catalog.json", "https://appassets.androidplatform.net/anything" }) {
            ...
            check("\"undefined\"".equals(js("typeof window.jazz4allHost")), "No bridge on foreign origin");

            // Same-origin frames may see the transport, but their messages must be rejected.
            String child = "<script>jazz4allHost.onmessage=()=>parent.postMessage('BAD_REPLY','*'); jazz4allHost.postMessage(JSON.stringify({type:'download',id:'frame',source:'unknown'})); parent.postMessage('FRAME_SENT','*');</script>";
            ...
            checks++;
            js("window.chartResult=''; (async()=>{const {parsePlaylist,savePlaylist}=await import('https://appassets.androidplatform.net/assets/v13/web/src/import.js'); const s=await import('https://appassets.androidplatform.net/assets/v13/web/src/storage.js'); const {renderSong}=await import('https://appassets.androidplatform.net/assets/v13/web/src/viewer.js'); const p=parsePlaylist('irealb://'+encodeURIComponent('Isolated test=Tester==Swing=C==[C |G7 Z==120=3')); const saved=await savePlaylist(p,null); const record=await s.getSong(saved.songIds[0]); const container=document.createElement('div'); renderSong(record,container,{transpose:2}); window.chartResult=container.querySelectorAll('irr-chord').length > 0 ? 'PASS':'FAIL';})().catch(e=>window.chartResult=e.message); true");
            ...
            Uri exportUri = Uri.parse("content://com.jazz4all.android.securitytest.export-test/playlist");
            ActivityMonitor cancelledPicker = exportPicker(new ActivityResult(Activity.RESULT_CANCELED, null));
            js("window.exportResult=''; window.jazz4allNative.savePlaylist('Practice.html','<html>étude ♭</html>').then(v=>window.exportResult=v,e=>window.exportResult=e.message); true");
            ...
            Uri badUri = Uri.parse("content://com.jazz4all.android.securitytest.export-test/missing");
            ActivityMonitor failedPicker = exportPicker(new ActivityResult(Activity.RESULT_OK, new Intent().setData(badUri)));
            js("window.exportResult=''; window.jazz4allNative.savePlaylist('Practice.html',window.exportText).then(v=>window.exportResult=v,e=>window.exportResult=e.message); true");
            await("window.exportResult.length > 0");
            check(js("window.exportResult").contains("Could not save"), "Provider write error reaches the UI");
            removeMonitor(failedPicker);
            js("window.cancelResult=''; const cancelTest=new AbortController(); window.jazz4allNative.fetchPlaylist('gypsy-jazz-denis-chang-early-recordings',cancelTest.signal).catch(e=>window.cancelResult=e.message); cancelTest.abort(); true");
            ...
            for (String[] collection : collections) {
                js("window.downloadResult=''; window.jazz4allNative.fetchPlaylist(" + JSONObject.quote(collection[0]) + ").then(async text=>{const {parsePlaylist}=await import('https://appassets.androidplatform.net/assets/v13/web/src/import.js'); window.downloadResult='songs:'+parsePlaylist(text).songs.length;}).catch(e=>window.downloadResult=e.message); true");
